# Unseen selector results

**判定：WEAK。** History 在 3 个 unseen targets 中赢 2 个、输 1 个。
...
| Target | TEMPLATE | TARGET | HISTORY | ORACLE | H/T ops |
| --- | --- | --- | --- | --- | --- |
| T2 | 356,404 | 278,703 | 203,167 | 155,237 | -27.10% |
| T3 | 258,504 | 296,355 | 240,257 | 183,849 | -18.93% |
| T4 | 281,185 | 234,747 | 249,696 | 178,214 | +6.37% |
...
| Target | 固定 offline calls | Offline discovery 秒 | Best online → oracle ops 改善 |
| --- | --- | --- | --- |
| T2 | 162 | 4.72 | 23.59% |
| T3 | 162 | 4.59 | 23.48% |
| T4 | 162 | 4.83 | 24.08% |
...
| Target | Route | Conflicts | 生成 ms | 选择/检索 ms | 验证 ms | Solver ms | Total ms |
| --- | --- | --- | --- | --- | --- | --- | --- |
| T2 | TEMPLATE | 9319 | 0.00 | 0.00 | 4.67 | 141.87 | 152.52 |
| T2 | TARGET | 7370 | 8.41 | 3.71 | 8.06 | 115.72 | 141.80 |
| T2 | HISTORY | 5377 | 8.23 | 4.53 | 8.04 | 72.18 | 98.69 |
| T2 | ORACLE | 4111 | 8.48 | 0.00 | 8.04 | 50.30 | 72.18 |
| T3 | TEMPLATE | 6980 | 0.00 | 0.00 | 5.11 | 102.67 | 113.20 |
| T3 | TARGET | 8333 | 8.60 | 3.68 | 8.01 | 122.06 | 148.67 |
| T3 | HISTORY | 6373 | 8.44 | 4.45 | 8.04 | 93.46 | 120.79 |
| T3 | ORACLE | 4848 | 8.69 | 0.00 | 8.10 | 61.46 | 83.82 |
| T4 | TEMPLATE | 7563 | 0.00 | 0.00 | 4.75 | 111.44 | 121.86 |
| T4 | TARGET | 6317 | 8.32 | 3.72 | 7.96 | 89.89 | 115.83 |
| T4 | HISTORY | 6651 | 8.28 | 4.44 | 8.05 | 95.57 | 122.41 |
| T4 | ORACLE | 4653 | 8.40 | 0.00 | 8.09 | 57.54 | 80.10 |
...
| Target | Resolver attempts A=B | Examined A=B | K A=B | 验证 steps A=B | History prior hits / 512 |
| --- | --- | --- | --- | --- | --- |
| T2 | 724 | 512 | 64 | 64 | 303 |
| T3 | 722 | 512 | 64 | 64 | 303 |
| T4 | 736 | 512 | 64 | 64 | 309 |
...
**Warm：T2/T3 能，T4 不能。三 target 合计仅节省 64.40ms。**
额外 history lookup 已计入选择时间；验证两边相同64条一步支持，差异很小。
T3 的 HISTORY 虽然优于 TARGET，仍比 TEMPLATE 的总时间慢；不是普遍值得物化。

| Target | H 比 T 少用时间 ms（负为更慢） | H/T total | H build 摊销后 total ms | 已有 prior 加载摊销后 total ms |
| --- | --- | --- | --- | --- |
| T2 | 43.11 | -30.40% | 1388.95 | 372.86 |
| T3 | 27.88 | -18.75% | 1411.06 | 394.96 |
| T4 | -6.59 | +5.69% | 1412.68 | 396.58 |

一次性 H proof 加载/检查/index/prior 计算为 **3.871s**；
已有完整 prior JSON 的加载和反序列化为 **0.823s**。
...
固定 TEMPLATE 的共同预加载成本 0.901s 对所有路线一致，另列而不混入 warm 对比。

若未来平均 warm 节省恰好保持本次 21.47ms/query，构建或加载的粗略
break-even 分别约 180 / 38 个查询。
**这是外推算式，不是已测得的摊销成功；T4 的负收益已说明不能假定每次都节省。**

## 4. 当前主线：ALIVE / WEAK / STOP？

**WEAK，不授予 ALIVE。** 事前 ALIVE 门槛要求三个 target 各自至少10% ops 改善、
