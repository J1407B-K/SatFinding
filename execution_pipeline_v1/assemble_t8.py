...
ROOT=Path(__file__).resolve().parent.parent; O=ROOT/'results/fresh_heuristic_causal_cohort_v1'; raw=O/'raw_t8'; cases=O/'cases';cases.mkdir(exist_ok=True)
...
for sdir in sorted([p for p in raw.iterdir() if p.name.startswith('S')],key=lambda p:int(p.name[1:])):
 st=json.load(open(sdir/'checkpoint.json')); act=json.load(open(sdir/'actions.json')); cid='T8F'+sdir.name[1:]; p=cases/cid
 ...
 st.update(case_id=cid,target='T8FRESH',state_id=f'T10FRESH_{sdir.name}',input_sha256=sha(cnf),binary_sha256=sha(binary),checkpoint_sha256=sha(sdir/'checkpoint.json'))
