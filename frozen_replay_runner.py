...
def verify(p):
 ...
 required={'state.json','actions.json','temporal_raw.json','temporal_features.json','logical.txt','heuristic.txt','input.cnf','protocol.json','feature_schema.json'}
 ...
 assert acts['selected_count']==2 and [x['rank'] for x in acts['selected']]==[1,2]
 assert len({x['id'] for x in acts['selected']})==2
 assert m['replay_binary_sha256']==sha(ROOT/'frozen_replay_native'),'BINARY_HASH'
...
def run(package,route,rank,output):
 ...
 assert route in ['baseline','action'];assert (route=='baseline' and rank is None) or (route=='action' and rank in [1,2])
 ...
 cmd=[str(ROOT/'frozen_replay_native'),str(p/'input.cnf'),str(o/'proof.drup'),str(o),str(o/'request.txt')]
 r=subprocess.run(cmd,capture_output=True,text=True,timeout=240)
 (o/'stdout.txt').write_text(r.stdout);(o/'stderr.txt').write_text(r.stderr);dump(o/'command.json',cmd)
 r.check_returncode();result=json.loads(r.stdout.splitlines()[-1]);assert result['status']=='UNSAT' and result['state_verified']
 ...
 proofcmd=[checker['path'],str(p/'input.cnf'),str(o/'proof.drup')];c=subprocess.run(proofcmd,capture_output=True,text=True,timeout=240)
