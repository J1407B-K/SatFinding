...
P=Path('results/heuristic_reverse_transplant')
B=Path('/private/tmp/satfinding-heuristic-reverse')
...
'''))
 ...
 hook=Path('heuristic_reverse_transplant.inc').read_text()
 ...
 shutil.copyfile(Path('/private/tmp/satfinding-decision828-l2-remove')/'B.cnf',B/'B.cnf')
 shutil.copyfile(Path('results/persistent_state_imprint/B_REMOVE_L2.before.allowed.bin'),P/'B_REMOVE_L2.before.allowed.bin')
 ...
 return dict(binary_sha256=sha(B/'run'),sources={str(p):sha(p) for p in [Path(__file__),Path('heuristic_reverse_transplant.inc'),B/'trace.inc',src]},checkpoint='first native pick with completed conflicts==660, qhead==trail.size()',experiment='one reverse joint transplant only',legality='candidate heap comparator/inverse indices and recipient eligible coverage checked BEFORE write; illegal candidate aborts, no repair')

def run(n):
 p=B/(n+'.drup'); inp=B/('A.cnf' if n in ['A','A_WITH_B_HEURISTIC'] else 'B.cnf')
 ...
 if n=='A_WITH_B_HEURISTIC':
  frozen=Path('results/persistent_state_imprint')
  ctrl=json.loads((frozen/'A.ps_before.snapshot.json').read_text());assert before==ctrl
  donor=json.loads((frozen/'B_REMOVE_L2.ps_before.snapshot.json').read_text())
  for field in ['activity','heap']: assert after[field]==donor[field]
  assert hashes['after']['allowed']==sha(frozen/'B_REMOVE_L2.before.allowed.bin')
  row['pre_state_matches_natural']=True
  # Match every recorded event before the hook, not only the checkpoint.
  def prefix(path,limit):
   return [v for v in map(json.loads,gzip.open(path,'rt')) if 'g' in v and v['g']<=limit]
  assert prefix(P/(n+'.events.jsonl.gz'),before['global'])==prefix(frozen/'A.events.jsonl.gz',before['global'])
  row['event_prefix_matches_A']=True
 dump(P/(n+'.result.json'),row);print(n,st,flush=True);return row

def main():
 assert not (P/'protocol.json').exists(),'frozen experiment exists'
 dump(P/'protocol.json',build())
 row=run('A_WITH_B_HEURISTIC');dump(P/'runs.json',{'A_WITH_B_HEURISTIC':row})
