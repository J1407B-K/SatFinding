"""Produce the bounded event diagnosis; performs no solver runs/interventions."""
import csv,gzip,hashlib,json
from pathlib import Path
P=Path('results/l1_l2_latent')
def read(n):return json.loads((P/n).read_text())
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
d=read('first_divergences.json');uses=read('l2_all_uses.json');stats=read('natural_runs.json')
with (P/'l2_direct_use_positions.csv').open('w') as f:
 w=csv.DictWriter(f,fieldnames=['type','conflict','decision','enqueue','global_event','local_event','literal'])
 w.writeheader()
 for t in ['E','A','M','C']:
  for r in uses[t]:w.writerow(dict(type=t,conflict=r['c'],decision=r['d'],enqueue=r['e'],global_event=r['g'],local_event=r['k'],literal=r['v']))
extra=[]
for folder,tag in [('', 'initialized'),('', 'first_reason_before'),('', 'first_reason_after'),('supplemental','U_199_7'),('', 'L_199_1'),('', 'ANALYZE_PRE_656_1'),('', 'D_PRE_657_1')]:
 a=read(f'{folder+"/" if folder else ""}A.{tag}.snapshot.json');b=read(f'{folder+"/" if folder else ""}B.{tag}.snapshot.json')
 extra.append(dict(checkpoint=tag,hashes={n:sha(P/folder/f'{n}.{tag}.snapshot.json') for n in ['A','B']},different=[k for k in a if a[k]!=b[k] and k!='global'],activity_variables=[i+1 for i,(x,y) in enumerate(zip(a['activity'],b['activity'])) if x!=y],heap_differences={k:sum(x!=y for x,y in zip(a['heap'][k],b['heap'][k])) for k in a['heap']},learned_ids_equal=[r[0] for r in a['learned']]==[r[0] for r in b['learned']],learned_metadata_differences=sum(x!=y for x,y in zip(a['learned'],b['learned'])),common_watches_equal=a['watches']==[[r for r in w if r[0]!='15289423184524811497'] for w in b['watches']]))
(P/'snapshot_detail.json').write_text(json.dumps(extra,indent=2))
protocol=read('protocol.json')
assert all(sha(k)==v for k,v in protocol['sources'].items())
assert read('natural_proof_checks.json')['A']['status']=='VERIFIED'
assert read('natural_proof_checks.json')['B']['status']=='VERIFIED'
assert stats['A']['analysis_resolution_steps']==345592 and stats['B']['analysis_resolution_steps']==169858
inter=read('intervention_result.json');assert inter['returncode']==14
assert not (P/'B_REASON_RESET.reason_reset_after.snapshot.json').exists()
b=read('B.first_reason_after.snapshot.json');ib=read('B_REASON_RESET.first_reason_after.snapshot.json');assert b==ib
# A small ledger validation checks the first-UIP causal substitution against the snapshots.
paths=read('critical_analysis_paths.json')
a199=next(r for r in paths if r['run']=='A' and r['conflict']==199)
b199=next(r for r in paths if r['run']=='B' and r['conflict']==199)
assert a199['analysis'][3]['clause']==[565,566,567]
assert b199['analysis'][3]['clause']==[-507,565,566]
assert a199['learned']['clause']==b199['learned']['clause']
assert next(r for r in extra if r['checkpoint']=='L_199_1')['activity_variables']==[567]
audit=dict(source_hashes_unchanged=True,native_totals_match_frozen=True,natural_proofs=read('natural_proof_checks.json'),intervention=dict(status='ABORTED_BEFORE_MUTATION',exit_code=14,reason='Existing original clause slot0 is not literal565; reason layout invariant fails.',snapshot_matches_natural_B=True,pre_attempt_sha256=sha(P/'B_REASON_RESET.first_reason_after.snapshot.json'),no_after_snapshot=True),artifact_sha256={n:sha(P/n) for n in ['A.events.jsonl.gz','B.events.jsonl.gz','first_divergences.json','l2_direct_use_positions.csv','snapshot_detail.json']})
(P/'audit.json').write_text(json.dumps(audit,indent=2))
rows=[]
labels={'reason':'reason / successful unit','analysis':'first-UIP antecedent','activity_bump':'activity vector','heap_at_bump':'heap array + indices','propagation_literal':'propagation literal','conflict_clause':'conflict clause','conflict_trail':'conflict trail','restart_queue':'restart internal queue','minimization':'minimization path','learned_content':'learned content','decision':'decision literal'}
for k in ['reason','analysis','activity_bump','heap_at_bump','propagation_literal','conflict_clause','conflict_trail','restart_queue','minimization','learned_content','decision']:
 r=d[k];a=r['A'];b=r['B'];rows.append(f"| {labels[k]} | {a['c']} / {b['c']} | {a['d']} / {b['d']} | {a['e']} / {b['e']} | {a['g']} / {b['g']} | {a['t']}:{a['k']} / {b['t']}:{b['k']} |")
table='\n'.join(rows)
report=f'''# L1 / L1+L2：晚激活，然后延迟分叉
...
'''
Path('L1_L2_LATENT_EFFECT_ANALYSIS.md').write_text(report)
print('Wrote L1_L2_LATENT_EFFECT_ANALYSIS.md and audit; all assertions passed')
